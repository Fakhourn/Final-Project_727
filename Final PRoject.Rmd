...
```{r}
res2<-gtrends(c("Free Palestine", "Genocide", "Hostages", "Protest"),
       geo="US",
...
```{r}
res5<-gtrends(c("Free Palestine", "Israel comitting Genocide", "Hostages", "Protest"),
       geo="US",
       time = "2023-9-30 2023-12-1",
       low_search_volume = TRUE)
```
...
```{r}
avg_search_longBS <- tidyr::gather(avg_searchbystate, key = "Keyword", value = "Average_Searchbystate", -Affl)
```
...
```{r}
ggplot(, aes(x = lat, y = long, group = Affl, fill = location$keyword)) + geom_polygon(color = "white", linewidth = 0.5) + scale_fill_manual(values = c("green", "blue", "purple", "yellow", "red"), name = "keyword") + theme_minimal() + labs(title = "Keywords")
```
...
```{r}
joined <- map_data %>%
  left_join(avg_searchbystate, by = c("location"))
```

```{r}
ggplot() +
  geom_polygon(data = avg_searchbystate, 
           aes(avg_searchbystate, x = long, y=lat, group = location),
           fill = "white", color = "black", size = 0.2) +
  geom_text_repel(data = avg_searchbystate,
                  aes(x = long, y = lat, label = "Free Palestine"),
                  size = 2, color = "blue") 
```

scale(avg_searchbystate$long,avg_searchbystate$lat)
```{r}
plot_usmap(location = "avg_searchbystate$'Pro Palestine Protest") + 
  labs(title = "US Pro Palestine Protest Keyword Searches",
       ) + 
...
```{r}
ggplot(ave_sbs_cluster_map) + 
 geom_polygon(data = ave_sbs_cluster_map, aes(x = long, y = lat, group= cluster, fill= cluster)) 
```

```{r}
ggplot(ave_sbs_cluster_map, aes(x = long , y = lat , group = cluster, fill = as.factor (cluster))) + geom_polygon(color = "black", linewidth = 0.5) + scale_fill_manual(values = c("#66c2a5", "#fc8d62", "#8da0cb", "yellow"), name = "Cluster") + theme_minimal() + labs(title = "US State ")
```

```{r}
ggplot() +
  geom_sf(data = ave_sbs_cluster_map, aes(fill = as.factor(cluster)))
```
